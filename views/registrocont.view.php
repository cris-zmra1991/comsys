<?php include_once '../templates/header_simple.php';?>
  <body>
    ...
    <table class="ui striped table">
      ...
      <tbody>
        <?php
        $data = $database->query("SELECT cont.id_fca, prov.desc_proveedor, cont.objeto_cont, cont.vigencia, cont.tipo_vigencia, cont.firma_cont, cont.valor, cont.no_contrato FROM contratos cont INNER JOIN com_proveedores prov ON cont.id_proveedor = prov.id_proveedor");

        foreach($data as $solicitudes){ ?>
          <tr>
            <td><?php echo $solicitudes["id_fca"] ?></td>
            <td><?php echo $solicitudes["desc_proveedor"] ?></td>
            <td><?php echo $solicitudes["objeto_cont"] ?></td>
            ...
            <td><a class="ui green button" href="<?php echo "details.php?name=" . $solicitudes["id_fca"]?>" target="container"><i class="edit icon"></i></a></td>
          </tr>
        <?php } ?>
      </tbody>
    </table>
